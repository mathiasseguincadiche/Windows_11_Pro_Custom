...
$details.CPU = $primaryCpu | Select-Object Name, Manufacturer, NumberOfCores, NumberOfLogicalProcessors, MaxClockSpeed, VirtualizationFirmwareEnabled
...
$details.Memory = @($memory | Select-Object Manufacturer, PartNumber, Capacity, Speed, ConfiguredClockSpeed)
...
$details.Motherboard = $board | Select-Object Manufacturer, Product, Version
...
$details.Video = @($video | Select-Object Name, DriverVersion, DriverDate, AdapterRAM, CurrentHorizontalResolution, CurrentVerticalResolution, CurrentRefreshRate)
...
$details.T705 = @($t705 | Select-Object FriendlyName, Model, MediaType, BusType, HealthStatus, OperationalStatus, Size)
...
$details.SystemDisk = $systemDisk | Select-Object Number, FriendlyName, BusType, PartitionStyle, OperationalStatus, HealthStatus, Size
...
try {
    ...
    $details.HardwareSymbiosisReport = 'reports\hardware\hardware-symbiosis.json'
} catch {
...
# Les objets CIM complets peuvent contenir des graphes profonds et produire un JSON tronqué.
# Le rapport n'embarque donc que des DTO explicites ci-dessus et toute future troncature devient bloquante.
try {
    $json = $report | ConvertTo-Json -Depth 8 -WarningAction Stop
} catch {
    throw "Sérialisation du rapport matériel non déterministe ou tronquée: $($_.Exception.Message)"
}
$json | Set-Content -Encoding utf8 $reportPath

# Relecture immédiate : un rapport écrit doit toujours être un JSON valide et exploitable.
try {
    $null = Get-Content -Raw -LiteralPath $reportPath | ConvertFrom-Json -ErrorAction Stop
} catch {
    throw "Rapport matériel JSON invalide après écriture: $reportPath | $($_.Exception.Message)"
}
...
foreach ($check in $checks.GetEnumerator()) {
    ...
    Write-Host ("[{0}] {1}" -f $state, $check.Key) -ForegroundColor $(if ([bool]$check.Value) { 'Green' } else { 'Red' })
}
foreach ($check in $advisoryChecks.GetEnumerator()) {
    $state = if ([bool]$check.Value) { 'INFO OK' } else { 'AVERTISSEMENT' }
    Write-Host ("[{0}] {1} | non bloquant" -f $state, $check.Key) -ForegroundColor $(if ([bool]$check.Value) { 'Green' } else { 'Yellow' })
}
