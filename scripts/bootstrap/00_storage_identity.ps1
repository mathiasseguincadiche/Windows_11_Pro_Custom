...
$canonicalBaselinePath = Join-Path $env:ProgramData 'Windows11ProCustom\storage-identity\volume-identity.json'
$legacyBaselinePath = Join-Path $env:ProgramData 'Windows11ProCustom\storage-v25\volume-identity.json'
if ([string]::IsNullOrWhiteSpace($BaselinePath)) {
    if (Test-Path -LiteralPath $canonicalBaselinePath) { $BaselinePath = $canonicalBaselinePath }
    elseif (Test-Path -LiteralPath $legacyBaselinePath) { $BaselinePath = $legacyBaselinePath }
    else { $BaselinePath = $canonicalBaselinePath }
}
$BaselineHashPath = "$BaselinePath" + '.sha256'
$reportDir = Join-Path $repoRoot 'reports\storage-identity'
...
function Get-WpcBaselineSchemaVersion {
    param([Parameter(Mandatory)]$Baseline)

    $schema = Get-WpcPropertyValue -InputObject $Baseline -Name 'SchemaVersion'
    if ($null -ne $schema) {
        $parsed = 0
        if ([int]::TryParse([string]$schema, [ref]$parsed)) { return $parsed }
        return -1
    }

    # Compatibilité de lecture uniquement avec l'ancien format de baseline.
    # Le numéro historique n'est plus une version du projet : il est traduit vers le schéma 1.
    $legacyContract = [string](Get-WpcPropertyValue -InputObject $Baseline -Name 'ContractVersion')
    if ($legacyContract -eq 'V25') { return 1 }
    return 0
}
...
function Get-WpcBaselineIntegrity {
    ...
    if (-not (Test-Path -LiteralPath $BaselinePath)) {
        throw "Baseline stockage absente: $BaselinePath"
    }
    ...
    if ($actual -ne $expected) {
        throw "Intégrité baseline stockage invalide. attendu=$expected actuel=$actual"
    }
...
if ($baselinePresent) {
    ...
    catch { $failures += "Baseline stockage illisible: $($_.Exception.Message)" }
    if ($null -ne $baseline) { $baselineSchemaVersion = Get-WpcBaselineSchemaVersion -Baseline $baseline }
    if ($Mode -in @('Audit', 'Verify')) {
        ...
        if ($null -ne $baselineIntegrity -and [string]$baselineIntegrity.Status -ne 'VERIFIED') {
            $failures += "Intégrité baseline stockage insuffisante: statut=$($baselineIntegrity.Status). Ré-enrôle explicitement la topologie saine pour générer le SHA-256 local."
        }
    }
}
...
if ($Mode -in @('Audit','Verify') -and $null -ne $baseline -and $null -ne $cPartition -and $null -ne $ePartition) {
    ...
    if ([int]$baselineSchemaVersion -ne 1) { $failures += "SchemaVersion baseline=$baselineSchemaVersion, attendue=1" }
    if ($null -eq $expectedC -or $null -eq $expectedE) {
        $failures += 'Schéma baseline stockage invalide: Roles.C et Roles.E sont obligatoires.'
    } else {
...
if ($Mode -eq 'Record') {
    ...
    if ($baselinePresent -and -not $ReplaceBaseline) {
        throw "Baseline stockage déjà présente: $BaselinePath. Aucun remplacement silencieux. Utilise -ReplaceBaseline avec -ConfirmHealthyTopology uniquement après investigation."
    }
    if ($failures.Count -gt 0) {
        throw "Enrôlement stockage refusé: stockage non qualifié. $($failures -join ' | ')"
    }
    ...
    Write-Host "[FAIT] Baseline stockage enregistrée explicitement: $BaselinePath" -ForegroundColor Green
    Write-Host "[FAIT] Sidecar SHA-256 enregistré: $BaselineHashPath ($baselineHash)" -ForegroundColor Green
    Write-Host 'Relance immédiatement le mode Verify avant toute installation.' -ForegroundColor Yellow
    return
}

if (-not $baselinePresent) {
    $message = "Baseline stockage absente. Après contrôle humain de C:/E:, exécute: .\scripts\bootstrap\00_storage_identity.ps1 -Mode Record -ConfirmHealthyTopology"
...
if ($failures.Count -gt 0) {
    if ($Mode -eq 'Verify') {
        throw "STORAGE_IDENTITY_BLOCK: topologie disque/volume différente ou non sûre. Aucune convergence autorisée. $($failures -join ' | ')"
    }
    Write-Host "[ALERTE] Topologie stockage non qualifiée: $($failures -join ' | ')" -ForegroundColor Yellow
} elseif ($baselinePresent) {
    Write-Host '[OK] C: et E: correspondent exactement aux identités physiques enrôlées.' -ForegroundColor Green
    Write-Host 'VERDICT: STORAGE IDENTITY READY' -ForegroundColor Green
}
Write-Host "[INFO] Rapport topologique: $reportPath" -ForegroundColor DarkGray
