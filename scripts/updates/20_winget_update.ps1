...
if (-not (Get-Command winget.exe -ErrorAction SilentlyContinue)) {
    throw 'winget.exe est introuvable. Installe ou répare App Installer depuis Microsoft Store.'
}
...
function Get-UpgradeInventory {
    ...
    if ($result.Code -ne 0 -and $result.Code -ne -1978335212) {
        throw ("winget list --upgrade-available a échoué (code={0}): {1}" -f $result.Code, ($result.Lines -join ' | '))
    }
...
function Write-Inventory {
    ...
    if ($Inventory.Rows.Count -eq 0) {
        Write-Host '[DÉJÀ OK] Aucune mise à jour WinGet détectée.' -ForegroundColor Green
        return
    }
...
if ($Mode -eq 'Verify') {
    if ($inventory.Rows.Count -gt 0) {
        throw ("{0} mise(s) à jour WinGet restent disponibles." -f $inventory.Rows.Count)
    }
    return
}
...
$inventory = Get-UpgradeInventory
if ($inventory.Rows.Count -eq 0) {
    Write-Host '[DÉJÀ OK] Applications WinGet déjà à jour après actualisation des sources.' -ForegroundColor Green
    return
}
