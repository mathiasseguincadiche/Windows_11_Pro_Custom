[CmdletBinding()]
param(
    [Parameter(Mandatory)][string]$BackupSessionPath,
    [ValidateSet('Verify','Sandbox')][string]$Mode = 'Verify',
    [string]$ScratchRoot,
...
function Assert-Hash {
    ...
    if ($actual -ne $Expected.ToUpperInvariant()) { throw "SHA-256 invalide pour $Path. attendu=$Expected actuel=$actual" }
}
...
foreach ($command in @('wsl.exe','wbadmin.exe')) { if (-not (Get-Command $command -ErrorAction SilentlyContinue)) { throw "Commande requise indisponible: $command" } }
...
if (-not ($isCanonical -or $isLegacy)) { throw 'Format de backup non supporté.' }
if ($isLegacy) { Write-Host "[COMPAT] Session historique utilisée sans modification: $SessionRoot" -ForegroundColor DarkGray }

$VhdxCandidates=@(Get-ChildItem -LiteralPath $WslDirectory -File -Filter '*.vhdx')
if ($VhdxCandidates.Count -ne 1) { throw "Le drill exige exactement un export WSL VHDX dans $WslDirectory. Détecté=$($VhdxCandidates.Count)" }
...
$StorageIdentityPath=Resolve-MetadataFile -Names @('storage-identity.json','storage-identity-v25.json')
$StorageIdentityHashPath=Resolve-MetadataFile -Names @('storage-identity.sha256','storage-identity-v25.sha256')
if (-not $StorageIdentityPath -or -not $StorageIdentityHashPath) { throw 'Preuve identité stockage ou sidecar SHA-256 absente.' }
...
if ([string]$Manifest.storageIdentity.sha256 -ne $ExpectedStorageHash) { throw 'Le SHA-256 identité stockage diverge entre manifest et sidecar.' }
...
$VersionsOutput=@(& wbadmin.exe get versions "-backupTarget:$BackupTargetDrive" 2>&1); $VersionsExitCode=$LASTEXITCODE; $global:LASTEXITCODE=0
if ($VersionsExitCode -ne 0 -or $VersionsOutput.Count -eq 0) { throw "wbadmin ne peut pas énumérer de version récupérable sur $BackupTargetDrive. code=$VersionsExitCode" }
$EnumeratedVersionIdentifiers=@(Get-WbadminVersionIdentifiers -Lines $VersionsOutput)
$ExpectedWbadminVersionIdentifier=if ($Manifest.PSObject.Properties.Name -contains 'wbadminVersionIdentifier') {[string]$Manifest.wbadminVersionIdentifier} else {''}
if (-not [string]::IsNullOrWhiteSpace($ExpectedWbadminVersionIdentifier)) {
    if ($EnumeratedVersionIdentifiers -notcontains $ExpectedWbadminVersionIdentifier) { throw "La version wbadmin liée au manifeste est introuvable. attendue=$ExpectedWbadminVersionIdentifier" }
...
Write-Host '[OK] SHA-256 de la baseline identité stockage valide.' -ForegroundColor Green
...
if ([string]::IsNullOrWhiteSpace($ScratchPathRoot)) { throw "Impossible de déterminer le volume du scratch depuis $ScratchRootFull" }
try {
    ...
    if ($ScratchDrive.AvailableFreeSpace -lt $RequiredScratchBytes) { throw "Espace scratch insuffisant. requis=$([math]::Round($RequiredScratchBytes/1GB,2)) Go disponible=$([math]::Round($ScratchDrive.AvailableFreeSpace/1GB,2)) Go" }
...
} catch { throw "Impossible de valider la capacité du scratch $ScratchPathRoot. $($_.Exception.Message)" }
...
if ($TemporaryDistribution -eq [string]$Manifest.wsl.distribution -or $TemporaryDistribution -eq 'Ubuntu') { throw 'Nom de distribution temporaire invalide: collision avec la distribution de production.' }
$DrillRoot=Join-Path $ScratchRootFull $TemporaryDistribution
$CopiedVhdx=Join-Path $DrillRoot 'ext4.vhdx'
$Imported=$false; $DrillFailure=$null; $CleanupFailures=New-Object System.Collections.Generic.List[string]
try {
    ...
    $ExistingNames=@(& wsl.exe --list --quiet 2>$null | ForEach-Object { ($_ -replace "`0", '').Trim() } | Where-Object { $_ }); $global:LASTEXITCODE=0
    if ($ExistingNames -contains $TemporaryDistribution) { throw "Collision inattendue avec une distribution existante: $TemporaryDistribution" }
    ...
    $ProbeExit=$LASTEXITCODE; $global:LASTEXITCODE=0
    if ($ProbeExit -ne 0) { throw "La distribution restaurée ne passe pas le probe Linux. code=$ProbeExit sortie=$($Probe -join ' | ')" }
    $ProbeText=$Probe -join ' | '
    if ($ProbeText -notmatch 'ID=ubuntu' -or $ProbeText -notmatch 'VERSION_ID=26\.04' -or $ProbeText -notmatch 'ext4') { throw "Le VHDX restauré ne correspond pas au contrat Ubuntu 26.04/ext4 attendu. sortie=$ProbeText" }
...
} catch { $DrillFailure=$_ }
finally {
    if ($Imported) {
        Write-Host "[INFO] Désenregistrement de la distribution temporaire $TemporaryDistribution" -ForegroundColor Yellow
        & wsl.exe --terminate $TemporaryDistribution 2>$null; $TerminateExitCode=$LASTEXITCODE; $global:LASTEXITCODE=0
        if ($TerminateExitCode -ne 0) { Write-Warning "wsl --terminate a échoué pour la distribution temporaire. code=$TerminateExitCode" }
        & wsl.exe --unregister $TemporaryDistribution 2>$null; $UnregisterExitCode=$LASTEXITCODE; $global:LASTEXITCODE=0
        if ($UnregisterExitCode -ne 0) { $CleanupFailures.Add("Le désenregistrement WSL a échoué pour $TemporaryDistribution. code=$UnregisterExitCode") }
        else {
            $RemainingNames=@(& wsl.exe --list --quiet 2>$null | ForEach-Object { ($_ -replace "`0", '').Trim() } | Where-Object { $_ }); $ListExitCode=$LASTEXITCODE; $global:LASTEXITCODE=0
            if ($ListExitCode -ne 0) { $CleanupFailures.Add("Impossible de confirmer le désenregistrement de $TemporaryDistribution. code=$ListExitCode") }
            elseif ($RemainingNames -contains $TemporaryDistribution) { $CleanupFailures.Add("La distribution temporaire reste enregistrée: $TemporaryDistribution") }
            else { $Imported=$false }
        }
    }
    if (-not $Imported -and (Test-Path -LiteralPath $DrillRoot)) {
        try { Remove-Item -LiteralPath $DrillRoot -Recurse -Force }
        catch { $CleanupFailures.Add("Impossible de supprimer la copie scratch $DrillRoot. $($_.Exception.Message)") }
    } elseif ($Imported) { Write-Warning "Copie scratch conservée pour éviter de supprimer le VHDX d'une distribution encore enregistrée: $DrillRoot" }
}
if ($null -ne $DrillFailure) { if ($CleanupFailures.Count -gt 0) { throw "$($DrillFailure.Exception.Message) Échec(s) de nettoyage: $($CleanupFailures.ToArray() -join ' | ')" }; throw $DrillFailure }
if ($CleanupFailures.Count -gt 0) { throw "Le drill a réussi, mais son nettoyage isolé a échoué: $($CleanupFailures.ToArray() -join ' | ')" }
