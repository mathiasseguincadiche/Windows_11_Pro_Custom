...
function Invoke-MainAction {
    ...
    switch ($Selected.ToLowerInvariant()) {
        '1' {if (Invoke-WpcPendingRebootGate -Context 'avant l installation complete') {return};if (Confirm-WpcAction -Message 'Lancer l installation COMPLETE de la workstation') {$completed=Invoke-WpcRepoScript -DisplayName 'Installation complete' -Path $InstallScript -Arguments @{Mode='Apply';FullInstall=[switch]::Present} -RequiresAdmin;if (-not $completed -and $script:LastActionRequiresReboot) {[void](Invoke-WpcPendingRebootGate -Context 'pour poursuivre l installation complete' -ForceRequired)}}}
...
function Show-ComponentsMenu {while ($true) {Write-Header;Write-Line ' COMPOSANTS SPECIFIQUES' White;Write-Line '';Write-Line '  1. WSL2 + stack DevOps + validation' White;Write-Line '  2. Qualification materielle guidee' White;Write-Line '  3. Audit empreinte SIMULATED' White;Write-Line '  4. Audit empreinte PHYSICAL' White;Write-Line '  5. Verifier la derive PHYSICAL' White;Write-Line '  6. Enregistrer la baseline PHYSICAL' White;Write-Line '  7. Remplacer la baseline PHYSICAL (archive + justification)' Yellow;Write-Line '  0. Retour' DarkGray;Write-Host '';$value=(Read-Host 'Ton choix').Trim();if ($value -eq '0') {return};if ($value -in @('1','2','3','4','5','6','7')) {Invoke-MainAction -Selected "8.$value";Pause-WpcMenu}}}
